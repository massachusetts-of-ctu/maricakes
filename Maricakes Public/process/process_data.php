...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($conn->query($sql) !== TRUE) {
        ...
        $ordID = (rand(100, 900)); 
        $sql = "INSERT INTO pre_order (r_id, order_id, customer_id, date_created, total) 
                VALUES ('$ordID', '$order_id', '$cId', '$date', '$grandTotal')";
        echo "Pre Orders successful\n";
         if ($conn->query($sql) !== TRUE) {
        ...
        foreach ($receipt_data['products'] as $product) {
            ...
            $sql = "INSERT INTO `pre_order_history`(`order_id`, `product_ids`, `qty`, `price`, `pro_total`, `ornum`, `subtotal`, `proname`, `tax`, `discount`, `total`, `customer_id`, `date`, `time`, `cashier`) 
                                        VALUES ('$order_id', '$productId', '$productQty', '$productPrice', '$proPrice', '$oror', '$productTotal', '$productName', '$tax', '$discount', '$grandTotal', '$cId', '$date', '$time', 'PRE-ORDER')";
            echo "Pre Order History Successful \n";
            if ($conn->query($sql) !== TRUE) {
