...
if (isset($_SESSION['username'])) {
	...
	$sql = "SELECT * FROM pre_order ORDER BY date_created DESC";
...
<html>
    <head>
        ...
        <title>MariCakes | Pre-Orders</title>
						...
						if (mysqli_num_rows($all_transaction) == 0) {
							...
							while ($row = mysqli_fetch_assoc($all_transaction)) {
								...
								<tr>
									...
									<td class="tbl-col-4">
										<button class="view edit-pre-order" onclick="openModals()"
											data-product-id="<?php echo $row["order_id"]; ?>">View</button>&nbsp;
										<button class="confirm-order confirm-pre-order" data-product-id="<?php echo $row["order_id"]; ?>"><i class="bi bi-check"></i></button>
										&nbsp;&nbsp;&nbsp;&nbsp;
										<button class="delete-order delete-pre-order" data-product-id="<?php echo $row["order_id"]; ?>"><i class="bi bi-x"></i></button>
									</td>
								</tr>
